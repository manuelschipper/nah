...
def _if_body_substitution_guard(
    raw_parts: list[str],
    stages: list[Stage],
    tail_parts: list[str],
) -> list[Stage]:
    placeholders = set(_placeholder_tokens_from_raw(raw_parts))
    if not placeholders:
        return []

    def guard() -> list[Stage]:
        return _control_flow_body_substitution_guard("if", raw_parts)

    dynamic_vars: set[str] = set()
    assigned_dynamic_vars: set[str] = set()
    accounted_placeholders: set[str] = set()

    for stage in stages:
        if not stage.tokens:
            continue
        if _stage_is_env_only_assignment(stage):
            if not _update_if_body_dynamic_assignments(
                stage,
                dynamic_vars,
                assigned_dynamic_vars,
                accounted_placeholders,
                placeholders,
            ):
                return guard()
            continue
        if _stage_command_position_is_dynamic(stage, dynamic_vars, placeholders):
            return guard()
        if not _stage_references_if_dynamic_data(stage, dynamic_vars, placeholders):
            continue
        if not _if_body_dynamic_use_is_safe(stage, dynamic_vars, placeholders):
            return guard()
        accounted_placeholders.update(_exact_stage_placeholders(stage, placeholders))

    if accounted_placeholders != placeholders:
        return guard()
    for name in assigned_dynamic_vars:
        if _raw_parts_reference_var(tail_parts, name):
            return guard()
    return []


def _stage_is_env_only_assignment(stage: Stage) -> bool:
    return (
        bool(stage.tokens)
        and stage.action_hint == taxonomy.FILESYSTEM_READ
        and stage.action_reason == "env-only assignment"
    )


def _update_if_body_dynamic_assignments(
    stage: Stage,
    dynamic_vars: set[str],
    assigned_dynamic_vars: set[str],
    accounted_placeholders: set[str],
    ...
    for token in stage.tokens:
        ...
        if value in placeholders:
            ...
            assigned_dynamic_vars.add(name)
            accounted_placeholders.add(value)
            continue

        ref_name = _exact_simple_var_reference(value)
        if ref_name is not None and ref_name in dynamic_vars:
            dynamic_vars.add(name)
            assigned_dynamic_vars.add(name)
            continue

        if _value_references_if_dynamic_data(value, dynamic_vars, placeholders):
            return False
        if name in dynamic_vars:
            if _safe_literal_var_value(value):
                dynamic_vars.remove(name)
            else:
                return False
    return True


def _exact_simple_var_reference(value: str) -> str | None:
    match = re.fullmatch(
...
def _stage_command_position_is_dynamic(
    stage: Stage,
    dynamic_vars: set[str],
    placeholders: set[str],
) -> bool:
    ...
    return _value_references_if_dynamic_data(command, dynamic_vars, placeholders)


def _stage_references_if_dynamic_data(
    stage: Stage,
    dynamic_vars: set[str],
    placeholders: set[str],
) -> bool:
    return any(
        _value_references_if_dynamic_data(value, dynamic_vars, placeholders)
        for value in [*stage.tokens, stage.redirect_target]
    )


def _value_references_if_dynamic_data(
    value: str,
    dynamic_vars: set[str],
    placeholders: set[str],
) -> bool:
    return bool(
        _placeholder_tokens_in_value(value, placeholders)
        or _dynamic_var_names_in_value(value, dynamic_vars)
        or _value_has_complex_dynamic_var_reference(value, dynamic_vars)
    )


def _placeholder_tokens_in_value(value: str, placeholders: set[str]) -> set[str]:
    return {placeholder for placeholder in placeholders if placeholder in value}


def _dynamic_var_names_in_value(value: str, dynamic_vars: set[str]) -> set[str]:
    names: set[str] = set()
    for match in _INTRA_CHAIN_VAR_RE.finditer(value):
...
def _value_has_complex_dynamic_var_reference(value: str, dynamic_vars: set[str]) -> bool:
    for name in dynamic_vars:
        if re.search(rf"\$\{{!{re.escape(name)}(?:\}}|[^}}]*\}})", value):
            return True
        if re.search(rf"\$\{{{re.escape(name)}[^}}]+\}}", value):
            return True
    return False
...
def _if_body_dynamic_use_is_safe(
    stage: Stage,
    dynamic_vars: set[str],
    placeholders: set[str],
) -> bool:
    if stage.redirect_target or stage.operator == "|":
        return False
    if not stage.tokens:
        return True
    ...
    if command == "printf":
        return _printf_dynamic_use_is_safe(stage, dynamic_vars, placeholders)
    if command in {"[", "test"}:
        return _test_dynamic_use_is_safe(stage, dynamic_vars, placeholders)
    return False


def _echo_dynamic_use_is_safe(
    stage: Stage,
    dynamic_vars: set[str],
    placeholders: set[str],
) -> bool:
...
def _printf_dynamic_use_is_safe(
    stage: Stage,
    dynamic_vars: set[str],
    placeholders: set[str],
) -> bool:
    ...
    return bool(dynamic_args) and all(
...
def _test_dynamic_use_is_safe(
    ...
    if len(args) == 2 and args[0] in {"-n", "-z"}:
        return _test_operand_dynamic_use_is_safe(args[1], dynamic_vars, placeholders)
    if len(args) == 3 and args[1] in {"=", "!="}:
        return (
            any(
                _value_references_if_dynamic_data(arg, dynamic_vars, placeholders)
                for arg in (args[0], args[2])
            )
            and _test_operand_dynamic_use_is_safe(args[0], dynamic_vars, placeholders)
            and _test_operand_dynamic_use_is_safe(args[2], dynamic_vars, placeholders)
        )
    return False


def _test_operand_dynamic_use_is_safe(
    token: str,
    dynamic_vars: set[str],
    placeholders: set[str],
) -> bool:
    if not _value_references_if_dynamic_data(token, dynamic_vars, placeholders):
        return True
    return _display_token_dynamic_use_is_safe(token, dynamic_vars, placeholders)


def _display_token_dynamic_use_is_safe(
    token: str,
    dynamic_vars: set[str],
    placeholders: set[str],
) -> bool:
    token_placeholders = _placeholder_tokens_in_value(token, placeholders)
    if token_placeholders and token not in token_placeholders:
        return False
    return not _value_has_complex_dynamic_var_reference(token, dynamic_vars)


def _exact_stage_placeholders(stage: Stage, placeholders: set[str]) -> set[str]:
    return {token for token in stage.tokens[1:] if token in placeholders}
...
def _consume_if(raw_stages: list[tuple[str, str]], start: int) -> tuple[list[Stage], int] | None:
    ...
    while j < len(raw_stages):
        ...
        if keyword == "fi" and not in_condition:
            ...
            tail_parts = [part for part, _op in raw_stages[j + 1:]]
            return _if_body_substitution_guard(executable_parts, stages, tail_parts) + stages, j + 1
