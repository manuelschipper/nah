...
They protect credential paths, auth stores, keychains, and caches. Cloud CLI
use alone does not imply file access. `secrets-env` blocks `.env` reads,
including copies, and direct output of catalogued credential environment
variables, but not environment presence checks, `.env` creation, or replacement.
