...
pub(super) fn protected_access_control_operation(
    program: &str,
    arguments: &[String],
    _cwd: Option<&str>,
    home: &str,
    ...
    arguments
        ...
        .and_then(|word| resolve_from_cwd(None, None, word, home, platform, true))
        .is_some_and(|target| protected_path_ancestor(&target, home, critical_paths, platform))
...
pub(super) fn protected_path_ancestor(
    path: &str,
    home: &str,
    critical_paths: &[AbsolutePath],
    platform: Platform,
    ...
    let ancestor_of = |candidate: &str| {
        ...
        path != candidate && path_contains(&path, &candidate)
    };
...
pub(super) fn resolve_from_cwd(
    ...
    if is_absolute(target, platform) {
        return Some(target.to_owned());
    }
    cwd.map(|cwd| join(cwd, target, platform))
}

fn join(base: &str, relative: &str, platform: Platform) -> String {
...
mod tests {
    ...
    #[test]
    fn protected_runtime_wiring_ancestors_are_recognized() {
        let protected =
            AbsolutePath::new(Platform::Linux, "/home/test/.kiro/hooks/nah.json").unwrap();
        assert!(protected_path_ancestor(
            "/home/test/.kiro/hooks",
            "/home/test",
            &[protected],
            Platform::Linux,
        ));
    ...
    #[test]
    fn literal_access_control_arguments_reach_ancestor_protection() {
        let protected =
            AbsolutePath::new(Platform::Linux, "/home/test/.kiro/hooks/nah.json").unwrap();
        let arguments = ["000".to_owned(), "/home/test/.kiro/hooks".to_owned()];
        assert_eq!(
