...
pub(crate) fn python_decodes_to_shell(program: &str, code: &str) -> bool {
    if !is_python_interpreter(&normalized_execution_program(program)) {
        return false;
    }
    let structure = python_structure(code);
    [
        "subprocess.run",
        "subprocess.call",
        "subprocess.Popen",
    ...
    .iter()
    .any(|name| {
        python_call_matches(&structure, name, |arguments| {
            let Some(arguments) = top_level_arguments(arguments) else {
                return false;
            };
            if arguments
                .iter()
                .filter(|argument| **argument == "shell=True")
                .count()
                != 1
            {
                return false;
            ...
            let named = arguments
                .iter()
                .filter_map(|argument| argument.strip_prefix("args="))
                .collect::<Vec<_>>();
            match named.as_slice() {
                [command] => {
                    arguments.iter().all(|argument| python_keyword(argument))
                        && decoded_python_expression(command)
                }
                [] => arguments.first().is_some_and(|command| {
                    decoded_python_expression(command)
                        && arguments[1..]
                            .iter()
                            .all(|argument| python_keyword(argument))
                }),
                _ => false,
            }
        })
    }) || ["os.system", "os.popen"].iter().any(|name| {
        python_call_matches(&structure, name, |arguments| {
            top_level_arguments(arguments).is_some_and(|arguments| {
                matches!(arguments.as_slice(), [command] if decoded_python_expression(command))
            })
        })
    })
}

fn python_keyword(argument: &str) -> bool {
...
fn python_structure(code: &str) -> String {
    let bytes = code.as_bytes();
    let mut structure = String::with_capacity(code.len());
    let mut index = 0;
...
fn python_call_matches(source: &str, name: &str, predicate: impl Fn(&str) -> bool) -> bool {
    source.match_indices(name).any(|(offset, _)| {
        ...
        let open = offset + name.len();
        boundary
            && !name.split_once('.').is_some_and(|(receiver, _)| {
                python_receiver_reassigned(&source[..offset], receiver)
            })
            && source.as_bytes().get(open) == Some(&b'(')
            && matching_python_delimiter(source, open)
                .is_some_and(|close| predicate(&source[open + 1..close]))
    })
}
...
fn matching_python_delimiter(source: &str, open: usize) -> Option<usize> {
...
fn top_level_arguments(arguments: &str) -> Option<Vec<&str>> {
...
fn decoded_python_expression(expression: &str) -> bool {
    ["base64.b64decode", "base64.urlsafe_b64decode"]
        .iter()
        .any(|decoder| {
            let Some(rest) = expression.strip_prefix(decoder) else {
                return false;
            };
            if rest.as_bytes().first() != Some(&b'(') {
                return false;
            }
            let Some(close) = matching_python_delimiter(rest, 0) else {
                return false;
            };
            if close == 1 {
                return false;
            ...
            let Some(decode) = tail.strip_prefix(".decode") else {
                return false;
            };
            decode.as_bytes().first() == Some(&b'(')
                && matching_python_delimiter(decode, 0) == Some(decode.len() - 1)
        })
}
