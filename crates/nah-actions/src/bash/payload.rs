...
use nah_inline::{EnvironmentValue, LanguageCallKind, LanguageDraft, NestedExecution};
...
use crate::bash_model::{
    FilesystemDraft, InvocationDraft, ProgramDraft, StageDraft, StdoutDraft, VariableValue,
};
...
impl Lowerer {
    ...
    pub(super) fn analyze_inline_stage(&mut self, execution: VisibleExecutionState) {
        ...
        self.append_language_draft(&execution, &program, &language_draft);
        for child in nested {
    ...
    fn append_language_draft(
        ...
        for (call_ordinal, call) in draft.calls().iter().enumerate() {
            ...
            self.stages.push(StageDraft {
                ...
                invocation_cwd: known_cwd(&execution.state).map(str::to_owned),
                filesystems,
