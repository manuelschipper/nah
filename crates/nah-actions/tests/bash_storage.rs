...
fn whole_backup_repository_deletion_emits_storage_destroy() {
    for source in [
        ...
        "borg delete --verbose --progress --stats --log-json /srv/backups/repo",
        "borg delete -v /srv/backups/repo",
...
fn snapshot_and_retention_deletion_emits_snapshot_delete() {
    for source in [
        ...
        "velero backup delete --all=false backup-1",
...
fn storage_controls_and_narrow_or_nonexecuting_forms_delegate() {
    for source in [
        ...
        "aws s3api delete-objects --bucket b --delete file://objects.json",
        ...
        "restic forget --unsafe-allow-remove-all=false",
...
fn dynamic_malformed_and_untrusted_executable_forms_are_not_claimed() {
    for source in [
        ...
        "rclone sync src ''",
        "rsync --delete src",
        "restic forget --unsafe-allow-remove-all=maybe",
        "velero backup delete --all=maybe",
