...
use crate::catalog::{shipped_guard_docs, shipped_guards, shipped_names};
use crate::live_state::{home, host_platform};
use crate::shipped_state::{ShippedState, set_enabled, state_path};
...
pub(crate) fn shipped_guard_entries() -> Result<Vec<GuardEntry>, String> {
    let platform = host_platform();
    let home = home(platform)?;
    let known = shipped_names();
    let state = ShippedState::load(&state_path(&home, platform), &known)
        .map_err(|error| error.to_string())?;
    Ok(shipped_guard_docs()
        ...
        .map(|guard| GuardEntry {
            ...
            path: None,
            status: if state.is_enabled(guard.name) {
                GuardStatus::Enabled
            } else {
