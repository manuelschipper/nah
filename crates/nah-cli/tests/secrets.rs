...
fn secret_guards_are_narrow_and_operation_sensitive_end_to_end() {
    ...
    for (command, guard) in [
        ...
        ("awk '1' ~/.ssh/id_rsa", "secrets-credentials"),
        ("sed -n '1p' ~/.ssh/id_rsa", "secrets-credentials"),
        ("xxd ~/.ssh/id_rsa", "secrets-credentials"),
        ("strings ~/.ssh/id_rsa", "secrets-credentials"),
        ("less ~/.ssh/id_rsa", "secrets-credentials"),
        ("cp ~/.ssh/id_rsa backup", "secrets-credentials"),
        (
            "python < ~/.gnupg/private-keys-v1.d/key",
            "secrets-credentials",
        ),
        ("sudo cat ~/.netrc", "secrets-credentials"),
        ("cat /etc/shadow", "secrets-credentials"),
        ("cat ~/.aws/credentials", "secrets-credentials"),
        (
            "cat ~/.config/gcloud/application_default_credentials.json",
            "secrets-credentials",
        ),
        ("cat ~/.docker/config.json", "secrets-credentials"),
        ("cat ~/.config/gh/hosts.yml", "secrets-credentials"),
        ("cat ~/.cargo/credentials.toml", "secrets-credentials"),
        ("cat ~/.npmrc", "secrets-credentials"),
        ("cat ~/.config/pypoetry/auth.toml", "secrets-credentials"),
        ("cat ~/.gem/credentials", "secrets-credentials"),
        ("cat ~/.config/glab-cli/config.yml", "secrets-credentials"),
        ("cat ~/.config/containers/auth.json", "secrets-credentials"),
        ("cat ~/.kube/config", "secrets-credentials"),
        ("cat ~/.aws/sso/cache/session.json", "secrets-credentials"),
        ("cat ~/.aws/cli/cache/session.json", "secrets-credentials"),
        ("cat ~/.config/gcloud/credentials.db", "secrets-credentials"),
        (
            "cat ~/.config/gcloud/access_tokens.db",
            "secrets-credentials",
        ),
        ("cat /home/alice/.aws/credentials", "secrets-credentials"),
        ("cat /etc/kubernetes/admin.conf", "secrets-credentials"),
        ("cat /etc/rancher/k3s/k3s.yaml", "secrets-credentials"),
        ("cat .env", "secrets-env"),
        ("git cat-file blob HEAD:.env", "secrets-env"),
        ("git cat-file -p HEAD:.ssh/id_rsa", "secrets-credentials"),
